...
public class DeathApplicationEnrichment {
    ...
    public void enrichDeathApplication(DeathRegistrationRequest deathRegistrationRequest) {
        List<String> deathRegistrationIdList = idgenUtil.getIdList(deathRegistrationRequest.getRequestInfo(), deathRegistrationRequest.getDeathRegistrationApplications().get(0).getTenantId(), "Taniya-dtr.registrationid", "", deathRegistrationRequest.getDeathRegistrationApplications().size());
        Integer index = 0;
...
//    }
    ...
    public void enrichApplicantOnSearch(
            final DeathRegistrationApplication application) {
        UserDetailResponse userResponse = userService.searchUser(
                userUtils.getStateLevelTenant(application.getTenantId()),
                String.valueOf(application.getApplicant().getId()), null);
...
//        log.info("user: " + user.toString());
        Applicant applicant = Applicant.builder()
                ...
                .gender(user.getGender()).id(user.getId())
                .name(user.getName()).type(user.getType())
